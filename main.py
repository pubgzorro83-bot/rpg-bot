...
def get_p(uid, name):
    if uid not in players:
        ...
        # Обновляем имя, если игрок поменял его в телеграме
        players[uid]["name"] = name
    return players[uid]

def send_menu(cid, text, shop_mode=False, name=None):
    # Добавляем имя игрока в начало сообщения для групп, если оно передано
    if name:
        text = f"👤 **{name}**, {text}"
    ...
    try:
        requests.post(BASE_URL + "sendMessage", json={"chat_id": cid, "text": text, "parse_mode": "Markdown", "reply_markup": keyboard}, timeout=10)
            ...
            for u in r["result"]:
                ...
                if "callback_query" in u:
                    ...
                    if data == "help":
                        send_menu(cid, 
                            ...
                            "👤 **Профиль** — Твоя статистика и снаряжение.", name=name
                        )
                    elif data == "profile":
                        send_menu(cid, 
                            f"твоя статистика:\n"
                            ...
                            f"🛡 Броня: {p['armor']}\n"
                            f"💰 Золото: {p['gold']}\n"
                            f"🍙 Онигири: {p['onigiri']} шт.", name=name
                        )
                    elif data == "top":
                        if not players:
                            send_menu(cid, "🏆 Список лидеров пуст!", name=name)
                        else:
                            ...
                            send_menu(cid, top_text, name=name)
                    elif data == "farm":
                        current_time = time.time()
                        if uid in last_farm_time and current_time - last_farm_time[uid] < 60:
                            left = int(60 - (current_time - last_farm_time[uid]))
                            send_menu(cid, f"⏳ Отдохни! На фарм можно ходить раз в минуту. Осталось: {left} сек.", name=name)
                        else:
                            ...
                            send_menu(cid, f"⛏ **Фарм завершен!**\n💰 Заработано монет: **+{g}**\n⭐ Опыт: +{xp_gain}" + ("\n🍙 Найдено: 1 Онигири!" if food else "") + f"\n💎 Всего золота: {p['gold']}" + lvl_up_text, name=name)
                    elif data == "dungeon":
                        if p['hp'] <= 25:
                            send_menu(cid, "⚠️ Слишком мало здоровья для данжа! Сначала подкрепись онигири.", name=name)
                        else:
                        ...
                        send_menu(cid, 
                            ...
                            "• 🛡 **Тяжелые доспехи** — 3000 монет", shop_mode=True, name=name
                        )
                    elif data == "buy_onigiri":
                        if p['gold'] >= 300:
                            p['gold'] -= 300
                            p['onigiri'] += 1
                            send_menu(cid, f"✅ Куплен 1 Онигири! Осталось золота: 💰 {p['gold']}", shop_mode=True, name=name)
                        else:
                            send_menu(cid, f"❌ Не хватает золота! Нужно 300 монет, а у тебя {p['gold']}.", shop_mode=True, name=name)
                    elif data == "buy_potion":
                        if p['gold'] >= 600:
                            p['gold'] -= 600
                            p['hp'] = p['max_hp']
                            send_menu(cid, f"✅ Зелье куплено и выпито! HP восстановлено. Золота: 💰 {p['gold']}", shop_mode=True, name=name)
                        else:
                            send_menu(cid, f"❌ Не хватает золота! Нужно 600 монет, а у тебя {p['gold']}.", shop_mode=True, name=name)
                    elif data == "buy_sword":
                        if p['gold'] >= 1500:
                            p['gold'] -= 1500
                            p['weapon'] = "Стальной меч"
                            send_menu(cid, f"✅ Куплен Стальной меч! Урон вырос. Золота: 💰 {p['gold']}", shop_mode=True, name=name)
                        else:
                            send_menu(cid, f"❌ Не хватает золота! Нужно 1500 монет, а у тебя {p['gold']}.", shop_mode=True, name=name)
                    elif data == "buy_armor":
                        if p['gold'] >= 3000:
                            p['gold'] -= 3000
                            ...
                            p['max_hp'] += 50
                            p['hp'] += 50
                            send_menu(cid, f"✅ Куплены Тяжелые доспехи! Макс. HP увеличено на +50. Золота: 💰 {p['gold']}", shop_mode=True, name=name)
                        else:
                            send_menu(cid, f"❌ Не хватает золота! Нужно 3000 монет, а у тебя {p['gold']}.", shop_mode=True, name=name)
                    elif data == "eat":
                        needed = random.randint(3, 5)
                        if p['hp'] >= p['max_hp']:
                            send_menu(cid, "⚠️ Здоровье и так полностью восстановлено!", name=name)
                        elif p['onigiri'] >= needed:
                            p['onigiri'] -= needed
                            p['hp'] = p['max_hp']
                            send_menu(cid, f"🍙 Ты плотно поел и потратил **{needed} шт.** онигири! HP полностью восстановлено до {p['max_hp']}. Осталось онигири: {p['onigiri']} шт.", name=name)
                        else:
                            send_menu(cid, f"❌ Не хватает онигири! Чтобы полностью восстановить здоровье, нужно съесть **{needed} шт.**, а у тебя только **{p['onigiri']} шт.**", name=name)
                ...
                # Обработка текстовых команд (в личке и группах)
                    ...
                    if txt in ["/start", "старт"]:
                        send_menu(cid, f"Привет! Добро пожаловать в экономическую RPG. Используй кнопки ниже:", name=name)
                        ...
                        send_menu(cid, f"твоя статистика:\n⭐ Уровень: {p['lvl']} (XP: {p['xp']}/{p['lvl'] * 100})\n❤️ Здоровье: {p['hp']}/{p['max_hp']}\n⚔️ Оружие: {p['weapon']}\n🛡 Броня: {p['armor']}\n💰 Золото: {p['gold']}\n🍙 Онигири: {p['onigiri']}", name=name)
                    elif txt in ["/top", "топ"]:
                        if not players:
                            send_menu(cid, "🏆 Список лидеров пуст!", name=name)
                        else:
